@if (count($landing_pages) > 0)
    @foreach ($landing_pages as $item)
        <tr class="tr-item item-level-{{ isset($level) ? $level : 0 }}">
            ...
            <td class="title">
                <a class="row-title " href="{{ route('admin.landing-page.edit', [$item->id]) }}">
                    <div><b style='color: #056FAD;'>{{ $item->name }} | {{ $item->name_en }}</b></div>
                </a>
                @if ($item->slug)
                    <div>
                        <b style='color:#777;'>URL VI:</b>
                        <a style='color:#00C600; word-break:break-all;' target='_blank' href="{{ route('page', ['slug' => $item->slug]) }}">
                            {{ route('page', ['slug' => $item->slug]) }}
                        </a>
                    </div>

                    <div>
                        <b style='color:#777;'>URL EN:</b>
                        <a style='color:#00C600; word-break:break-all;' target='_blank' href="{{ route('page', ['slug' => $item->slug], true, 'en') }}">
                            {{ route('page', ['slug' => $item->slug], true, 'en') }}
                        </a>
                    </div>
                @endif
            </td>
