@php
    $campaign = \App\Models\Backend\Post::where(['status' => 1, 'type' => 'campaign'])
        ->orderByDesc('sort')
        ->get();
@endphp
...
@if (count($campaign) > 0)
    @foreach ($campaign as $item)
        <div class="form-group">
            <label for="campaign_{{ $item->id }}" class="">
                <input type="checkbox" class="campaign_item_input" value="{{ $item->id }}" id="campaign_{{ $item->id }}">
                {{ $item->name }}
