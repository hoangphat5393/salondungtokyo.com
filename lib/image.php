...
class image{
	...
	function check_file($file_name){
		...
		$allow_types = array('jpg','jepg','png','gif','bmp');

		$ext = pathinfo($file_name, PATHINFO_EXTENSION);
		if (!in_array($ext, $allow_types)) {
		    return 'Chỉ nhận file ảnh jpg, jepg, png, gif, bmp';
		}
		return 1;
	}
	...
	function upload($file_tmp_name, $file_path, $file_new_name, $width='',$height='', $method='fixed'){

		// Tiến hành upload ảnh đại diện
		// $thumb_name = time().'_'.rand(100000, 999999).'.'.$ext;

		if(move_uploaded_file($file_tmp_name, $file_path.$file_new_name)){
			if($width || $height){
				$this->resize_image($method,$file_path.$file_new_name,$file_path.$file_new_name,$width,$height);
			}
			return 1;
		}
	...
	// Hàm này sẽ tạo ra một hình với hình ảnh được thu nhỏ chính xác với kích thước bạn chỉ định.
	function resize_image_fixed($image,$max_width,$max_height) {
	...
	function resize_image($method,$image_loc,$new_loc,$width,$height) {
	    ...
	    if (!in_array($method,array('force','max','crop','fixed'))) { $GLOBALS['errors'][] = 'Invalid method selected.'; }
	    ...
	    if ($method == 'crop') { $new_image = $this->resize_image_crop($image,$width,$height); }
	    elseif ($method == 'max') { $new_image = $this->resize_image_max($image,$width,$height); }
	    elseif ($method == 'fixed') { $new_image = $this->resize_image_fixed($image,$width,$height); }
	    elseif ($method == 'force') { $new_image = $this->resize_image_force($image,$width,$height); }
